...
export default function Home() {
  ...
  const kenyaLocations: { [key: string]: string[] } = {
    "Nairobi": ["Westlands", "Kasarani", "Makadara", "Langata"],
    "Kiambu": ["Thika", "Ruiru", "Kiambu Town"],
    "Machakos": ["Machakos Town", "Mwala"],
  ...
  const handlePayment = async () => {
    ...
    setLoading(true);
    const deposit = 25000; // temporary fixed amount for testing
    ...
    const handler = window.PaystackPop.setup({
      ...
      callback: async (response: any) => {
        await supabase.from("bookings").insert({ ...formData, deposit_paid: true, paystack_reference: response.reference });
      ...
      <nav className="fixed top-0 w-full bg-black/95 backdrop-blur z-50 border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <Image src="/dk-logo.png" alt="DK" width={48} height={48} />
            <div>
              <span className="text-3xl font-bold">DK DIGITAL LENS</span>
              <p className="text-xs text-amber-300">capturing the moments that matter to you</p>
            </div>
          </div>
          <div className="text-emerald-400 font-mono">{currentTime}</div>
        </div>
      </nav>

      <section className="min-h-screen pt-20 flex items-center justify-center bg-gradient-to-br from-black to-zinc-900">
        <div className="text-center">
          <h1 className="text-7xl font-bold">DK DIGITAL LENS</h1>
          <p className="text-4xl text-amber-300 mt-4">Book Your Coverage</p>
        </div>
      </section>
          ...
          <div className="bg-zinc-900 rounded-3xl p-10">
            ...
            <form className="space-y-8">
              ...
              <button
                ...
                disabled={loading}
                className="w-full bg-emerald-600 hover:bg-emerald-500 py-6 rounded-3xl text-xl font-semibold"
              >
